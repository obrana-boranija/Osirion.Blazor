...
/// </summary>
public class FileSystemContentRepository : RepositoryBase<ContentItem, string>, IContentRepository
{
    ...
    public async Task<IReadOnlyList<ContentItem>> FindByQueryAsync(ContentQuery query, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                // Since Directory is a property on ContentItem, we can filter by it if it's set
                filteredItems = filteredItems.Where(item =>
    ...
    public async Task<ContentItem> SaveWithCommitMessageAsync(ContentItem entity, string commitMessage, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(directoryPath) && !Directory.Exists(directoryPath))
            {
                ...
                {
                    throw new Domain.Exceptions.DirectoryNotFoundException($"Directory does not exist: {directoryPath}");
                }
            }
    ...
    private string ExtractLocaleFromPath(string path)
    {
        ...
        var contentPath = NormalizePath(_options.ContentRoot ?? string.Empty);
