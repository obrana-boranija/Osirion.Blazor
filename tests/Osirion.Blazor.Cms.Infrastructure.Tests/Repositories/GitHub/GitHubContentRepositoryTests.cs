...
namespace Osirion.Blazor.Cms.Infrastructure.Tests.Repositories.GitHub
{
    public class GitHubContentRepositoryTests
    {
        ...
        public async Task GetAllAsync_ReturnsGitHubContents()
        {
            ...
            _markdownProcessor.ExtractFrontMatterAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
                .Returns(callInfo =>
                {
                    var content = callInfo.Arg<string>();
                    if (content.Contains("Post 1"))
                        return Task.FromResult((new Dictionary<string, string> { { "title", "Post 1" } }, "Content 1"));
                    else
                        return Task.FromResult((new Dictionary<string, string> { { "title", "Post 2" } }, "Content 2"));
                });
